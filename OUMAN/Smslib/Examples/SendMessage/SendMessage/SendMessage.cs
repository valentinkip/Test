...
namespace SendMessage
{
	class SendMessage
	{
		public class CallNotification : ICallNotification
		{
			...
			{
				Console.WriteLine(">>> New call detected from Gateway: " + gatewayId + " : " + callerId);
			}
		}

		public class InboundNotification : IInboundMessageNotification
		{
			public void process(String gatewayId, org.smslib.Message.MessageTypes msgType, InboundMessage msg)
			{
				if (msgType == org.smslib.Message.MessageTypes.INBOUND) Console.WriteLine(">>> New Inbound message detected from Gateway: " + gatewayId);
				else if (msgType == org.smslib.Message.MessageTypes.STATUSREPORT) Console.WriteLine(">>> New Inbound Status Report message detected from Gateway: " + gatewayId);
		...
		{
			public void process(String gatewayId, org.smslib.AGateway.GatewayStatuses oldStatus, org.smslib.AGateway.GatewayStatuses newStatus)
			{
				Console.WriteLine(">>> Gateway Status change for " + gatewayId + ", OLD: " + oldStatus + " -> NEW: " + newStatus);
			}
		}
		...
		static void Main(string[] args)
		{
			...
			try
			{
				...
				IPModemGateway gateway = new IPModemGateway("com4", "127.0.0.1", 12000, "Nokia", "XPress Music");
				...
				Console.WriteLine("  Signal Level: " + gateway.getSignalLevel() + "%");
				Console.WriteLine("  Battery Level: " + gateway.getBatteryLevel() + "%");
				Console.WriteLine();

				// Send one message.
				// Remember to change the recipient!
				OutboundMessage msg = new OutboundMessage("+306948494037", "Hello from SMSLib for .NET");
				srv.sendMessage(msg);
				Console.WriteLine(msg);
